...
class CourseGenerationService:
            ...
            if start != -1 and end != -1:
                ...
                try:
                    ...
                    try:
                        ...
                        print("Warning: Could not parse JSON response. Creating fallback structure.")
                        return {
                            "content": [
                                {
                ...
                # Create a fallback structure
                print("Warning: Could not extract JSON. Creating fallback structure.")
                return {
                    "content": [
                        {
                            "subsection_title": "Generated Content", 
    ...
    def generate_course(self, topic: str, num_sections: int = 5, model: str = "llama-3.3-70b-versatile") -> Dict:
        ...
        """
        print(f"Step 1/3: Generating course outline...")
        ...
        try:
            ...
            print(f"Warning: Failed to parse course outline. Error: {str(e)}")
            print("✓ Creating simplified course structure...")
        ...
        full_course = course_outline.copy()
        
        print(f"Step 2/3: Generating content for {len(full_course['sections'])} sections...")
        for i, section in enumerate(full_course["sections"]):
            print(f"  Generating content for section {i+1}/{len(full_course['sections'])}: {section['title']}...")
            ...
            try:
                ...
                print(f"  ✓ Content generated for section {i+1} with {len(section['content'])} subsections")
            except Exception as e:
                print(f"  Warning: Failed to parse content for section {i+1}. Error: {str(e)}")
                print(f"  ✓ Creating placeholder content for section {i+1}")
        ...
        print("Step 3/3: Finalizing course structure...")
        return full_course
    ...
    def generate_accessible_course(self, 
        ...
        """
        print(f"Generating course on '{topic}' with {num_sections} sections...")
        course = self.generate_course(topic, num_sections, model)
        
        print("Saving course to files...")
        output_path = self.save_course_to_files(course, output_dir)
        
        print(f"Course generated successfully and saved to {output_path}/")
